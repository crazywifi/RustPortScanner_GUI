...
fn run_gui() {
    ...
    let upload_counter: Arc<AtomicUsize> = Arc::new(AtomicUsize::new(0));
    ...
    loop {
        ...
        let uc = Arc::clone(&upload_counter);
        thread::spawn(move || serve_request(request, sf, pf, ls, uc));
    }
}
...
fn serve_request(
    ...
    match (method.as_str(), url_path.as_str()) {
        ...
        // ── Upload target file (streamed, no memory limit) ────────────────────
        // POST /api/upload  with raw body = text lines (IPs/CIDRs)
        // Optional header X-Append-File: <path>  to append to existing temp file (folder mode)
        // Returns {"path":"<tmpfile>","lines":<count>}
        ("POST", "/api/upload") => {
            let append_path = request.headers().iter()
                .find(|h| h.field.as_str().to_string().to_lowercase() == "x-append-file")
                .map(|h| h.value.as_str().to_string());

            let tmp_path = if let Some(ref p) = append_path {
                p.clone()
            } else {
                let id = upload_counter.fetch_add(1, Ordering::SeqCst);
                let tmp_dir = env::temp_dir();
                tmp_dir.join(format!("ps_upload_{}.txt", id))
                    .to_string_lossy().into_owned()
            };

            // Stream body → temp file in 64KB chunks, counting valid lines
            let result: Result<usize, String> = (|| {
                let mut file = if append_path.is_some() {
                    fs::OpenOptions::new().append(true).open(&tmp_path)
                        .map_err(|e| format!("Cannot open {}: {}", tmp_path, e))?
                } else {
                    fs::File::create(&tmp_path)
                        .map_err(|e| format!("Cannot create temp file: {}", e))?
                };
                let mut buf_reader = io::BufReader::with_capacity(65536, request.as_reader());
            ...
            match result {
                ...
                Err(e) => {
                    request.respond(
                        Response::from_string(serde_json::json!({"error": e}).to_string())
